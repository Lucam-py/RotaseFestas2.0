...
const TicketForm: React.FC<IProps> = ({ data }) => {
  ...
  const handleDecrease = (ticket: IData): void => {
    const tickets: IData[] = formValues.filter(
      (e: IData) => e.id !== ticket.id,
    );
  ...
  const handleSubmit = async (
    ...
    if (quantity > 0) {
      ...
      showAlert({
        type: 'error',
        text: 'Você deve selecionar pelo menos 01 ingresso para este evento.',
      });
    }
  };

  if (loading) {
    return <Loader type="inline" color="gray" text="Espere um segundo." />;
  }
      ...
      <div className="ticket-box-buttons">
        {formValues.length > 0 ? (
          ...
          <Button
            type="submit"
            color="disabled"
            text="Ingressos não encontrados."
          />
        )}
      </div>
    </form>
